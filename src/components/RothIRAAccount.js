...
export function AddRothIRAAccount() {
    const [balance, setBalance] = useState('');
    const [accHolderid, setAccHolderid] = useState('');

    const ROTH_IRA_ACCOUNT_URL = process.env.REACT_APP_API_ENDPOINT+"accountholder/{accHolderid}/rothiraaccounts";
    ...
    const handleSubmit = async (event) => {
        ...
        await fetch(ROTH_IRA_ACCOUNT_URL.replace('{accHolderid}', accHolderid), {
        ...
        .then(res=> {
            if(res.ok){
                ...
                alert(`Successfully opened a new Roth IRA Account for account holder: ${accHolderid}`);
                return res;
            } else {
...
export function GetRothIRAAccount() {
        ...
        <div className="container">
            ...
            <div className="row">
                <div className="col-md-7">
                    <Card>
                        <h2 className="text-center">Roth IRA Account List</h2>
                        <Form onSubmit={handleSubmit} className="mt-3">
                            <FormGroup className="col-md-6" >
...
export function CloseRothIRAAccount(){
    const [accHolderid,setAccHolderid] = useState("");
    const [message, setMessage] = useState("");
    //const [accNumber, setAccNumber] = useState("");
    const bearer = 'Bearer ' + localStorage.getItem('token');

    const CLOSE_ROTH_IRA_ACCOUNT_URL = process.env.REACT_APP_API_ENDPOINT+"accountholder/{id}/rothiraaccount";
    const handleSubmit=async(event)=>{
        event.preventDefault();
        await fetch(CLOSE_ROTH_IRA_ACCOUNT_URL.replace('{id}',accHolderid),{
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': bearer
    ...
    return(
        <div className="container ">
            ...
            <div className="row">
                <div className="col-md-7">
                    <Card>
                        <h2 className="text-center">Close Roth IRA Account </h2>
                        <Form onSubmit={handleSubmit} className="mt-3">
                            <FormGroup className="col-md-6" >
                                <Label for="accHolderid">Enter the AccountHolder-Id</Label>
                                <Input type="accHolderid" name="accHolderid" required
