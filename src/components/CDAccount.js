...
export function CloseCDAccount(){
    const [accHolderid,setAccHolderid] = useState("");
    ...
    const CLOSE_CD_ACCOUNT_URL = process.env.REACT_APP_API_ENDPOINT+"accountholder/{id}/closecdaccount/{accountNum}";
    const handleSubmit=async(event)=>{
        event.preventDefault();
        await fetch(CLOSE_CD_ACCOUNT_URL.replace('{id}/closecdaccount/{accountNum}',`${accHolderid}/closecdaccount/${accNumber}`),{
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': bearer
    ...
    return(
        <div className="container ">
            ...
            <div className="row">
                <div className="col-md-7">
                    <Card>
                        <h2 className="text-center">Close CD Account </h2>
                        <Form onSubmit={handleSubmit} className="mt-3">
                            <FormGroup className="col-md-6" >
                                <Label for="accHolderid">Enter the AccountHolder-Id</Label>
                                <Input type="accHolderid" name="accHolderid" required
                            ...
                            <FormGroup className="col-md-6" >
                                ...
                                <Input type="accNumber" name="accNumber" required
                                    id="accNumber" placeholder=" AccountHolder-Number" value={accNumber}
